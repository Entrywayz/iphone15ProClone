...
const VideoCarousel = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      if (anim) gsap.ticker.remove(animUpdate)
    }
  ...
  const handleProcess = (type, i) => {
    switch (type) {
      ...
      case 'pause':
        ...
        break
      case 'change-video':
        setVideo((pre) => ({...pre, videoId: i, isPlaying: true}))
        break
      default: 
        return video
    }
  ...
  return (
    <>
      ...
      <div className='relative flex-center mt-10'>
        <div className='flex-center py-5 px-7 bg-[#42424570] backdrop-blur rounded-full'>
          {hightlightsSlides.map((_, i) => (
            <span 
              key={i} 
              ref={(el) => (videoDivRef.current[i] = el)} 
              className='mx-2 w-3 h-3 bg-[#42424570] rounded-full relative cursor-pointer'
